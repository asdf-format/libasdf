...
option(USE_LIBSTATGRAB "Use libstatgrab for memory info" ON)
option(LIBSTATGRAB_NO_PKGCONFIG "Detect libstatgrab without using pkg-config" NO)
if(LIBSTATGRAB_NO_PKGCONFIG)
    set(LIBSTATGRAB_LIBRARIES "statgrab")
    set(LIBSTATGRAB_LIBDIR "" CACHE STRING "Directory containing libstatgrab library")
    set(LIBSTATGRAB_INCLUDEDIR "" CACHE STRING "Directory containing libstatgrab headers")
    set(LIBSTATGRAB_CFLAGS "" CACHE STRING "Compiler options for libstatgrab")
    set(LIBSTATGRAB_LDFLAGS "" CACHE STRING "Linker options for libstatgrab")
    link_directories(${LIBSTATGRAB_LIBDIR})
    include_directories(${LIBSTATGRAB_INCLUDEDIR})
    add_link_options(${LIBSTATGRAB_LDFLAGS})
    add_compile_options(${LIBSTATGRAB_CFLAGS})
else()
    if(PKG_CONFIG_FOUND)
        pkg_check_modules(STATGRAB libstatgrab REQUIRED)
    else()
        message("pkg-config not found. Install pkg-config, or use LIBSTATGRAB_NO_PKGCONFIG=YES.")
    endif()
endif()
