...
MU_TEST(block_create_set_append) {
    ...
    asdf_block_t *block = asdf_block_create(data, len);
    assert_not_null(block);
...
MU_TEST(block_data_alloc_roundtrip) {
    ...
    size_t n = 256;
    asdf_block_t *block = asdf_block_create(NULL, n);
    assert_not_null(block);
...
MU_TEST(block_create_destroy) {
    ...
    asdf_block_t *block = asdf_block_create(NULL, 128);
    assert_not_null(block);
    ...
    asdf_block_t *empty = asdf_block_create(NULL, 0);
...
MU_TEST(block_data_set_compressed) {
    ...
    asdf_block_t *block2 = asdf_block_create(NULL, 0);
