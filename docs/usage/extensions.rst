...
and ``asdf_value_as_<type>`` functions, as well as ``asdf_<type>_copy``,
``asdf_<type>_copy_into``, ``asdf_<type>_deinit`` and ``asdf_<type>_destroy``.
...
The ``deinit`` method must free any memory owned by the ``asdf_foo_t``'s
fields, but **must not free the object itself**--its storage may be embedded
in another struct, an array element, or static.  It is passed a `void *` to the
``asdf_foo_t`` object.  Freeing the object's own storage is handled by the
generated ``asdf_foo_destroy`` (which calls ``deinit`` and then ``free``);
``asdf_foo_deinit`` exposes just the field cleanup for caller-owned storage:

.. code:: c

  static void asdf_foo_deinit(void *value) {
...
If the extension object is shallow--that is, none of its fields require
dynamically allocated memory--it is also possible to leave the ``deinit``
method undefined (it is automatically reduced to zeroing the object's memory).

The ``asdf_foo_copy`` function is used to implement the user-level
``asdf_foo_copy`` / ``asdf_foo_copy_into`` API generated by the extension.  This
is needed for most extension values that contain nested data, so that a deep
copy of the object can be created correctly.  The copy method fills the
caller-provided ``dst`` (which the generated wrappers have already zeroed) and
returns ``true`` on success or ``false`` on failure (the only assumed failure
mode being out-of-memory).  On failure the wrapper de-initializes the partial
``dst`` via ``deinit`` for you, so the copy method needs no cleanup of its own:

.. code:: c

  static bool asdf_foo_copy(asdf_file_t *file, const void *src, void *dst) {
